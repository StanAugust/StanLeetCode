...
public class Subsequence {
	...
	public int lengthOfLIS(int[] nums) {
		...
		for (int i = 0; i < nums.length; i++) {
			for (int j = 0; j < i; j++) {
				if (nums[j] < nums[i]) {
					dp[i] = Math.max(dp[i], dp[j] + 1); // 既然是递增子序列，只要找到前面那些结尾比 num[i]小的子序列，然后把
														// num[i]接到最后，就可以形成一个新的递增子序列，而且这个新的子序列长度加一
				}
			}
	...
	public int maxEnvelopes(int[][] envelopes) {
		...
		/*
		 * 先对宽度w进行升序排序，如果遇到w相同的情况，则按照高度h降序排序。之后把所有的h作为一个数组，在这个数组上计算 LIS 的长度就是答案。 关键点：
